...
tap('SPACE'); assert.equal(run('scene'),'intro');
// Segurar espaço não pula a introdução iniciada pela mesma tecla.
for(let i=0;i<20;i++) tick({SPACE:1});
...
tap('SPACE'); tap('SPACE'); assert.equal(run('scene'),'playing');
run('player.x=0; player.z=-26; player.y=6.1;');
tick(); assert.equal(run('scene'),'victory');
